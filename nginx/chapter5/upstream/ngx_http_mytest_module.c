...
mytest_upstream_process_header(ngx_http_request_t *r)
{
    ...
    for ( ;; )
    {
        // http框架提供了基础性的ngx_http_parse_header_line 方法，它用于解析http头部
        ...
        //要求upstream模块继续接收新的字符流再交由process_header 回调方法解析
        if (rc == NGX_AGAIN)
        {
            return NGX_AGAIN;
        }
...
ngx_http_mytest_handler(ngx_http_request_t *r)
{
    ...
    // samuel: 因为 h_addr_list 指向的是static数组。这里应该copy一份的。否则后续gethostbyname 会改变这个值?
